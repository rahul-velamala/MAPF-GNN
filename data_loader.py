        ...
        for case in pbar_load:
            ...
            try:
                ...
                if not (state_data.shape[1] == traj_data.shape[0] == gso_data.shape[1] == self.nb_agents):
                    # Optionally print mismatch details for debugging
                    # print(f"Debug: Skipping {case}. Agent mismatch. Expected: {self.nb_agents}. "
                    #       f"Got state N={state_data.shape[1]}, traj N={traj_data.shape[0]}, gso N={gso_data.shape[1]}")
                ...
                if not (state_data.shape[0] == gso_data.shape[0] == num_steps + 1):
                     # Optionally print mismatch details for debugging
                     # print(f"Debug: Skipping {case}. Time mismatch. Actions T={num_steps}. "
                     #       f"Expected States/GSO T+1 = {num_steps+1}. "
                     #       f"Got State[0]={state_data.shape[0]}, GSO[0]={gso_data.shape[0]}")
...
#     # Example config - UPDATE PATHS AND PARAMETERS TO MATCH YOUR DATA
#     test_config = {
#         "train": {
#             "root_dir": "dataset/5_8_28/train", # <<< CHANGE TO YOUR ACTUAL TRAIN DATA PATH
#             "mode": "train",
#             "nb_agents": 5,                     # <<< CHANGE TO YOUR # AGENTS
#             "min_time": 5,                      # <<< Adjust filter
#             "max_time_dl": 55,                  # <<< Adjust filter (use 'max_time_dl')
#         },
#         "valid": {
#             "root_dir": "dataset/5_8_28/val",   # <<< CHANGE TO YOUR ACTUAL VALIDATION DATA PATH
#             "mode": "valid",                    # 'valid' not 'val' for key lookup
#             "nb_agents": 5,                     # <<< CHANGE TO YOUR # AGENTS
#             "min_time": 5,                      # <<< Adjust filter
#             "max_time_dl": 55,                  # <<< Adjust filter
#         },
#         "num_agents": 5,                        # Global fallback if 'nb_agents' missing in train/valid
#         "batch_size": 4,
#         "num_workers": 0, # Use 0 for easier debugging first
#         "min_time": 1,    # Global fallback filter
#         "max_time_dl": 60 # Global fallback filter
#     }
...
#             count = 0
#             for i, batch_data in enumerate(data_loader_instance.train_loader):
#                 try:
#                     batch_s, batch_t, batch_g = batch_data
...
#                      print(f"Error unpacking train batch {i}: {e}. Items received: {len(batch_data)}")
#                 count += 1
#                 if count >= 2: break
#         else:
#              print("\nTrain loader is empty or could not be created.")
#
#         if data_loader_instance.valid_loader and len(data_loader_instance.valid_loader.dataset) > 0:
#             print("\n--- Iterating through valid_loader (first 2 batches) ---")
#             count = 0
#             for i, batch_data in enumerate(data_loader_instance.valid_loader):
#                 try:
#                     batch_s, batch_t, batch_g = batch_data
...
#                     print(f"  GSO Shape:    {batch_g.shape}, Type: {batch_g.dtype}")
#                 except ValueError as e:
#                      print(f"Error unpacking valid batch {i}: {e}. Items received: {len(batch_data)}")
#                 count += 1
#                 if count >= 2: break
#         else:
#              print("\nValid loader is empty or could not be created.")
#
#     except Exception as e:
